		...
		for (const comp of module_meta) {
			...
			const exports: (string | any)[][] = [
				[
					...
					[
						// The "shared" generation renders custom components with the app's
						// own runtime, so there is nothing to hand a `mount` over — the
						// extra entry would only ship a runtime the app never asks for.
						...(generation === "bridge"
