...
import { plugins, make_gradio_plugin, detect_generation } from "./plugins";
...
logger.error = (msg, options) => {
	// Upstream swallows "Pre-transform error" here. Those are real failures — a
	// module that fails to transform leaves the app hanging with no hint at all,
	// so they are worth the noise.
	...
	try {
		const server = await createServer({
			...
			server: {
				port: frontend_port,
				host: host,
				// The app is served by the dev server, but whatever the backend owns
				// has to stay reachable under the same origin: the app resolves
				// `theme.css` against the document, so without this the dev server
				// answers with its HTML fallback and the app renders unstyled.
				proxy: {
					"/theme.css": backend_url,
					"/manifest.json": backend_url,
					"/custom_component": backend_url,
					"/gradio_api": { target: backend_url, ws: true }
				},
				fs: {
...
async function generate_imports(
	...
	const runtimes =
		detect_generation(root) === "bridge"
			? component_entries.reduce((acc, component) => {
					// The "bridge" app mounts each component with its own Svelte
					// runtime instance. This eager `import("svelte")` is only ever read
					// through the `virtual:cc-init` module, where Vite rewrites the
					// bare specifier to a real URL. It must never be inlined into the
					// page as-is.
					return `${acc}"${component.component_class_id}": import("svelte"),\n`;
				}, "")
			: // The "shared" app renders components with its own runtime and never
				// reads `__GRADIO__CC__RUNTIMES__`, so leave it empty — emitting the
				// bare `import("svelte")` here would break the inlined bootstrap.
				"";
