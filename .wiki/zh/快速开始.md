...
**重要更新**：ModelScope Studio 2.0.0-beta.1 已迁移到 Gradio 6.0 生态系统，支持版本范围为 >=6.0.0 且 <=6.8.0。
...
- Python 环境与版本
  ...
  - **重要说明**：当前示例使用 5.34.1，但 2.0.0-beta.1 版本已迁移到 Gradio 6.0 生态系统
- 前端依赖
  - Ant Design 6.x、Ant Design X 2.x、React 19、Svelte 5、@gradio/\* 系列等
- 文档与示例依赖
  - 示例脚本与文档站点使用 modelscope_studio==2.0.0-beta.1
...
ModelScope Studio 2.0.0-beta.1 已完成从 Gradio 4.x 到 6.x 的重大迁移，涉及以下关键变更：
...
> **❗ Beta 版本稳定性风险提示**
> `2.0.0-beta.1` 为预发布版本，API 可能尚未正式稳定，部分功能在后续版本中可能发生变更。生产环境建议等待正式版发布后再升级；如需提前体验新特性，请充分评估变更日志并备份当前环境。
...
- **Gradio 版本冲突**
  ...
  - 如果使用新版本 Gradio，请升级到 modelscope_studio 2.0.0-beta.1
